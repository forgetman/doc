...
public class IncrementalProcessor extends AbstractProcessor {
    ...
    private final TreeMap<String, IncrementalType> processors = new TreeMap<>();
    ...
    private void processAnnotations(RoundEnvironment roundEnv) {
        ...
        for (Element e : roundEnv.getElementsAnnotatedWith(Incremental.class)) {
            if (!checkAnnotatedElement(e, processor)) {
                continue;
            }
            IncrementalType processorType = e.getAnnotation(Incremental.class).value();
            if (processorType == IncrementalType.DYNAMIC
    ...
    private void generateConfigFiles() {
        ...
        try {
            ...
            try (PrintWriter out =
                ...
                processors.forEach((processor, type) -> out.println(processor + "," + type.name().toLowerCase()));
                if (out.checkError()) {
