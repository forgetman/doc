package feature.media.player.base
...
abstract class BasePlayer(
    ...
    private val ignoreAudioFocusLoss: Boolean
) : SimpleBasePlayer(Looper.getMainLooper()), feature.media.player.Player {

    companion object {
        private const val LOG_TAG = "BasePlayer"
    }
    ...
    private val playerControl = object : AudioFocusManager.PlayerControl {
        override fun setVolumeMultiplier(volumeMultiplier: Float) {
            L.d(LOG_TAG, "setVolumeMultiplier, volumeMultiplier = $volumeMultiplier")
            this@BasePlayer.volume = volumeMultiplier
        }

        override fun executePlayerCommand(playerCommand: Int) {
            handlePlayerControlCommand(playerCommand)
        }
    }
            ...
            val newSession = MediaSession.Builder(context, this)
                ...
                .setCallback(object : MediaSession.Callback {
                    override fun onMediaButtonEvent(
                        ...
                        L.d("onMediaButtonEvent, intent = $intent, controllerInfo = $controllerInfo")
                        return super.onMediaButtonEvent(session, controllerInfo, intent)
                    }
                })
                .build()
            newSession.player = this
    ...
    protected fun updatePlayWhenReady(
         ...
         * 由于playWhenReady置为false了, 所以还会回调一次多余的PLAYER_COMMAND_DO_NOT_PLAY
         *
         * 只有临时失去过焦点, 才允许恢复播放
         * 只有[C.USAGE_MEDIA]和[C.USAGE_GAME]允许恢复播放
         */
        when (playerCommand) {
            ...
            AudioFocusManager.PLAYER_COMMAND_WAIT_FOR_CALLBACK -> {
                if (this.playWhenReady) {
                    if (attributes.usage == C.USAGE_MEDIA || attributes.usage == C.USAGE_GAME) {
    ...
    protected open fun handlePlayerControlCommand(playerCommand: Int) {
        if (ignoreAudioFocusLoss && playerCommand == AudioFocusManager.PLAYER_COMMAND_DO_NOT_PLAY) {
            L.d(LOG_TAG, "handlePlayerControlCommand, ignoreAudioFocusLoss = true, playerCommand = $playerCommand")
            return
        }
