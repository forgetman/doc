...
public final class AudioFocusManager {
    ...
    /**
     * 用于指示音频焦点的增益或未知持续时间的音频焦点请求。当其他应用申请焦点之后又释放焦点会触发此回调。
     *
    ...
    /**
     * 用于指示临时增益或音频焦点请求，预计持续时间短，比如notification sounds
     *
    ...
    /**
     * 用于指示音频焦点的临时请求，预计持续时间较短时间量，以及其他音频应用程序可以继续播放的位置降低产出水平后（也称为“回避”）。
     *
    ...
    /**
     * 用于指示音频焦点的临时请求，预计持续时间较短，没有其他应用程序或系统组件应该播放的时间量、任何东西。
     * 独占和瞬态音频焦点请求的例子是语音、备忘记录和语音识别，在此期间系统不应该播放任何内容通知，媒体播放应暂停。
     *
     ...
     */
    public AudioFocusManager(Context context, Handler eventHandler, @Nullable PlayerControl playerControl) {
        this.audioManager =
                checkNotNull(
...
//          "Automatic handling of audio focus is only available for USAGE_MEDIA and USAGE_GAME.");
    ...
    private boolean shouldAbandonAudioFocusIfHeld(@Player.State int playbackState) {
        return playbackState == Player.STATE_IDLE || playbackState == Player.STATE_ENDED || focusGainToRequest == AUDIOFOCUS_NONE;
    }
